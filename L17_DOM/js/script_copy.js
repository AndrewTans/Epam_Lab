(function() {
    ...
    function filterArray() {
        students.map(function(el) {
            return studentsNew.push({
                "Student": `${el.name} ${el.lastName}`,
                "email": `${el.email}`,
                "Profile Picture": `${el.img}`,
                "Skills": el.skills
            });
        });
        ...
        for (let i = 0; i < studentsNew.length; i++) {
            ...
            for (let key in studentsNew[i]) {
                ...
                if (key === "Profile Picture") {
                    td[j].innerHTML = (`<img src="${studentsNew[i][key]}" style="width: 150px;">`);
                } else {
                    td[j].innerHTML = studentsNew[i][key];
                }
            ...
            td[j] = document.createElement('td');
            // td.id = 'edit';
            td[j].className = 'edit glyphicon glyphicon-edit';
            tr[i].appendChild(td[j]);
            j++
            // remove
            td[j] = document.createElement('td');
            // td.id = 'remove';
            td[j].className = 'remove glyphicon glyphicon-trash';
    ...
    function clickAlert() {
        ...
        function show(event) {
            ...
            // targetElement .style.backgroundColor = '#F00'; // use it
            event.stopPropagation();
            if (event.target.parentNode.childNodes[2] === undefined) {
                console.log(event.target.parentNode.previousSibling.previousSibling.textContent);
            } else {
        ...
        if (targetElement.className.match('edit')) {
            ...
            for (let i = 0; i < studentsNew.length; i++) {
                ...
                if (studentsNew[i]['email'] === studentEmail.textContent) {
                    console.log(studentEmail.textContent);
                    temp_num = i;
                }
