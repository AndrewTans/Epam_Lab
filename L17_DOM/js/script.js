    ...
    function makeTable() {
        ...
        for (let i = 0; i < studentsNew.length; i++) {
            ...
            for (let key in studentsNew[i]) {
                ...
                if (key === "Profile Picture") {
                    td.innerHTML = (`<img src="${studentsNew[i][key]}" style="width: 150px;">`);
                } else {
                    td.innerHTML = studentsNew[i][key];
                }
                tr[i].appendChild(td);
            }
            // controls start
            td = document.createElement('td');
            td.className = 'edit glyphicon glyphicon-edit';
            tr[i].appendChild(td);
            // remove
            td = document.createElement('td');
            td.className = 'remove glyphicon glyphicon-trash';
